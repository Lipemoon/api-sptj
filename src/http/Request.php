...
class Request {
    ...
    function __construct(string $uri, string $method, ?string $rawBody = null) {
        ...
        //se não existe /api/ no path, mantém inteiro (fallback)
        if ($route === $path) {
        // tenta remover uma possível barra inicial
            $route = ltrim($path, '/');
        }       
        ...
        // Segmentos tradicionais preservados:
        $this->resource = $segments[0] ?? null;
        $this->id = $segments[1] ?? null;
        $this->subCollection = $segments[2] ?? null;
